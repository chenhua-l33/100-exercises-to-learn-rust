...
impl Display for TicketNewError {
    fn fmt(&self, f: &mut fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            TicketNewError::DescriptionError(error) => write!(f,"{}","Description not provided".to_string()),
        }
    }
}
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()){
        ...
        Err(err) => match err {
            ...
            TicketNewError::DescriptionError(_) => Ticket{title: title.clone(), description: "Description not provided".into(), status: status.clone()},
        }
     }
}
